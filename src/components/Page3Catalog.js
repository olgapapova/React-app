...
import { booksList3Load } from "../redux/booksList3Load.js";
import  BooksListCategory  from './BooksListCategory'; 
import { choosePage } from '../redux/chooseSlice';
import './PageCatalog.css';

export const Page3Catalog = () => {
  const dispatch = useDispatch();

  useEffect (
    ()=> {
    dispatch( booksList3Load );
  },[])

  useEffect (
    ()=> {
    dispatch( choosePage(3) );
  },[])
  ...
  return (
    <>
      <div className="Navigation">
        <NavLink to="/">Главная</NavLink><span> &gt; Каталог &gt; 3</span>
      </div>
